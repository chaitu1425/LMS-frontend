            ...
            <table className='min-w-full text-sm'>
                ...
                <tbody >
                    ...
                    <tr key={index} className='border-b hover:bg-gray-50 transition duration-200'>
                        ...
                        <td className='px-4 py-3'><FaEdit className='text-gray-600 hover:text-blue-600 cursor-pointer' onClick={()=>navigate(`/editcourse/${course?._id}`)}  /></td>
                    </tr>
                    ))}
                </tbody>
            </table>
        ...
        <div className='md:hidden space-y-4' >
            ...
            <div key={index} className='bg-white rounded-lg shadow p-4 flex flex-col gap-3'>
                <div className='flex gap-4 items-center'>
                    ...
                    {course?.thumbail?<img src={course?.thumbail} alt="" className='w-16 h-16 rounded-md object-cover' />:<img src={img} alt="" className='w-16 h-16 rounded-md object-cover' />}
                ...
                <span className={`w-fit px-3 py-1 text-xs rounded-full ${course?.isPublished ?"bg-red-100  text-green-600":"bg-red-100 text-red-600"} `}>{course?.isPublished? "Published" : "Draft"}</span>
            </div>
            ))}
