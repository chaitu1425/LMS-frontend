        ...
        <div className='min-h-screen bg-gradient-to-br from-black to-gray-900 text-white flex flex-col items-center px-4 py-16'>
            ...
            <div className='bg-white shadow-xl rounded-3xl p-6 sm:p-8 w-full max-w-2xl text-center relative'>
                ...
                <div className='flex items-center bg-gray-700 rounded-full overflow-hidden shadow-lg relative w-full'>
                    ...
                    {input && <button className='absolute right-14 sm:right-16 bg-white rounded-full'><img src={img} className='w-10 h-10 p-2 rounded-full' alt="" onClick={() => handleRecommendation(input)} /></button>}
                    <button className='absolute right-2 bg-white rounded-full w-10 h-10 flex items-center justify-center' onClick={handleSearch}><RiMicAiFill className='w-5 h-5 text-[#cb87c5]' /></button>
                </div>
            </div>
            {
                recommendation.length > 0 ? (
                    ...
                    listening ? <h1 className='text-center text-xl sm:text-2xl mt-10 text-gray-400'>
                        Listening...
                    </h1> :
